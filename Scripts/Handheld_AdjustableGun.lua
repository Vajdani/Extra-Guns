...
function Handheld.client_onCreate( self )
    ...
    self.effect = sm.effect.createEffect( "Template Highlight" )
    ...
    self.shootEffect = self.effects[1][1]
	self.shootEffectFP = self.effects[1][2]
    ...
    if not sm.game.getEnableAmmoConsumption() or sm.container.canSpend( sm.localPlayer.getInventory(), obj_plantables_potato, 1 ) then
        ...
        if owner then
            for i = 1, self.data.shots do
                local dir = sm.noise.gunSpread( dir, self.data.spread )
                sm.projectile.projectileAttack( projectileTypes[self.data.type].projectile, self.data.damage, firePos, dir * self.data.fireForce, owner, fakePosition, fakePositionSelf ) 
            end
        end
        ...
        self.lastShotTick = sm.game.getCurrentTick()
    else
        local fireMode = self.aiming and self.aimFireMode or self.normalFireMode
        self.fireCooldownTimer = fireMode.fireCooldown
...
function Handheld:cl_template()
	if templates ~= nil and #templates > 0 and not self.data.isTemplate then
...
--[[
function Handheld:client_canInteract()
...
function Handheld:client_onFixedUpdate( dt )
    self.shootEffect = self.effects[self.data.type][1]
    self.shootEffectFP = self.effects[self.data.type][2]

    if self.data.fullAuto and self.isFiring and self.tool:isEquipped() then
        self.fireCounter = self.fireCounter + dt
        if sm.game.getCurrentTick() >= self.lastShotTick + self.data.delay then
            self.fireCounter = 0
            self:cl_shoot()
        end
    end
...
function Handheld.client_onUpdate( self, dt )
	...
	if self.gui:isActive() then
		...
		if not self.data.isTemplate then
            local txt = templates ~= nil and #templates > 0 and self.data.templateCount > 0 and templates[self.data.templateCount].name or "none"
	...
	if self.effect:isPlaying() then
		...
		self.effect:setPosition( sm.localPlayer.getPlayer():getCharacter():getTpBonePos( "pejnt_barrel" ) )
	end
